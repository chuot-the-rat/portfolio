...
const PreviewPanel = ({ project }) => {
    // Determine layout type based on project category and available images
    const isMobileApp = project.category?.toLowerCase().includes('mobile');
    const isWebDesign = project.category?.toLowerCase().includes('web');
    ...
    return (
        <motion.div
            className={`preview-panel preview-layout-${isMobileApp ? 'mobile' : isWebDesign ? 'desktop' : 'mixed'}`}
