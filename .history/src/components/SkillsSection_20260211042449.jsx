...
const asciiScripts = {
    ...
    // Individual skills
    "User Research": {
        lines: ["> user.study()", "  qualitative + quantitative"],
        animType: "cube",
    },
    Wireframing: {
        lines: ["> sketch.wireframe()", "  structure before style"],
        animType: "brackets",
    },
    Prototyping: {
        lines: ["> proto.build()", "  test early, fail cheap"],
        animType: "loading",
    },
    "User Testing": {
        lines: ["> test.observe()", "  watch, don't assume"],
        animType: "dots",
    },
    "User Flows": {
        lines: ["> flow.map()", "  every path, every exit"],
        animType: "arrows",
    },
    "Journey Mapping": {
        lines: ["> journey.trace()", "  pain points to fixes"],
        animType: "arrows",
    },
    Figma: {
        lines: ["> figma.open()", "  components, variants, auto-layout"],
        animType: "arrows",
    },
    Typography: {
        lines: ["> font.set()", "  hierarchy through type"],
        animType: "dots",
    },
    "Color Theory": {
        lines: ["> color.harmonize()", "  contrast, meaning, mood"],
        animType: "cube",
    },
    Accessibility: {
        lines: ["> a11y.check()", "  wcag, focus, contrast"],
        animType: "brackets",
    },
    "Responsive Design": {
        lines: ["> layout.adapt()", "  fluid, not fixed"],
        animType: "loading",
    },
    "Design Systems": {
        lines: ["> sys.organize()", "  tokens, patterns, rules"],
        animType: "cube",
    },
    "HTML/CSS": {
        lines: ["> markup.render()", "  semantic, styled, shipped"],
        animType: "arrows",
    },
    JavaScript: {
        lines: ["> js.execute()", "  logic, events, data"],
        animType: "brackets",
    },
    React: {
        lines: ["> react.render()", "  components all the way down"],
        animType: "cube",
    },
    "Next.js": {
        lines: ["> next.build()", "  ssr, routing, api"],
        animType: "loading",
    },
    Python: {
        lines: ["> python.run()", "  scripting, data, tools"],
        animType: "dots",
    },
    "Git/GitHub": {
        lines: ["> git.commit()", "  branch, merge, ship"],
        animType: "brackets",
    },
    "Adobe Illustrator": {
        lines: ["> vector.draw()", "  paths, shapes, exports"],
        animType: "arrows",
    },
    Photoshop: {
        lines: ["> px.edit()", "  raster, retouch, composite"],
        animType: "loading",
    },
    "After Effects": {
        lines: ["> anim.keyframe()", "  motion, timing, easing"],
        animType: "arrows",
    },
    Framer: {
        lines: ["> motion.animate()", "  interactive prototypes"],
        animType: "cube",
    },
    "VS Code": {
        lines: ["> editor.code()", "  extensions, shortcuts, flow"],
        animType: "loading",
    },
    Notion: {
        lines: ["> notes.sync()", "  docs, boards, wikis"],
        animType: "dots",
    },
};
...
export default function SkillsSection({ variant = "grid" }) {
    ...
    /* ── ASCII Panel state (Chaos Mode sidebar) ── */
    const [panelScript, setPanelScript] = useState(asciiScripts._intro);
    const [panelFrame, setPanelFrame] = useState(0);
    const panelInterval = useRef(null);
    const hoverDebounce = useRef(null);
    const activeCategoryRef = useRef(null);

    /* Start / restart the panel animation loop */
    const startPanelAnim = useCallback((script) => {
        setPanelScript(script);
        setPanelFrame(0);
        if (panelInterval.current) clearInterval(panelInterval.current);
        const frames =
            asciiAnimations[script.animType] || asciiAnimations.arrows;
        panelInterval.current = setInterval(() => {
            setPanelFrame((prev) => (prev + 1) % frames.length);
        }, 150);
    }, []);

    /* Debounced panel update */
    const updatePanel = useCallback(
        (scriptKey) => {
            if (hoverDebounce.current) clearTimeout(hoverDebounce.current);
            hoverDebounce.current = setTimeout(() => {
                const script = asciiScripts[scriptKey] || asciiScripts._intro;
                startPanelAnim(script);
            }, 80);
        },
        [startPanelAnim],
    );

    /* Reset panel to category or intro */
    const resetPanel = useCallback(() => {
        if (hoverDebounce.current) clearTimeout(hoverDebounce.current);
        hoverDebounce.current = setTimeout(() => {
            const fallback = activeCategoryRef.current
                ? asciiScripts[activeCategoryRef.current] || asciiScripts._intro
                : asciiScripts._intro;
            startPanelAnim(fallback);
        }, 80);
    }, [startPanelAnim]);

    /* Kick off intro animation when section enters viewport */
    useEffect(() => {
        if (isVisible && !isWorkMode) {
            startPanelAnim(asciiScripts._intro);
        }
        return () => {
            if (panelInterval.current) clearInterval(panelInterval.current);
            if (hoverDebounce.current) clearTimeout(hoverDebounce.current);
        };
    }, [isVisible, isWorkMode, startPanelAnim]);
    ...
    const panelFrames =
        asciiAnimations[panelScript.animType] || asciiAnimations.arrows;
    ...
    return (
        ...
        >
            ...
            <div className="skills-layout">
                {/* ASCII Panel */}
                <div className="skills-ascii-panel">
                    <div className="skills-ascii-panel-inner">
                        {panelScript.lines.map((line, i) => (
                            <div
                                key={i}
                                className="skills-ascii-line"
                            >
                                {line}
                            </div>
                        ))}
                        <div className="skills-ascii-anim-row">
                            <span className="skills-ascii-frame">
                                {panelFrames[panelFrame % panelFrames.length]}
                            </span>
                        </div>
                    </div>
                </div>
