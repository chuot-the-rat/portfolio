...
const Home = () => {
    ...
    useEffect(() => {
        try {
            // Load projects from centralized case studies
            const caseStudyProjects = getAllProjects();
            
            // Load projects.json for folder paths and additional metadata
            fetch("/projects.json")
                .then((res) => res.json())
                .then((projectsList) => {
                    // Merge case study data with projects.json metadata
                    const projectsWithData = caseStudyProjects.map((caseStudyProject, index) => {
                        const projectMeta = projectsList.find(p => p.id === caseStudyProject.id) || {};
                        ...
                        return {
                            ...
                            systemMetadata: projectMetadata[caseStudyProject.id] || {
                                status: "active",
                                role: "ui/ux",
                            ...
                            hoverImages: [
                                ...(caseStudyProject.solution?.images?.slice(1, 3) || []),
                                ...(caseStudyProject.overview?.images?.slice(1, 2) || []),
                            ].filter(Boolean),
                        };
                    });
