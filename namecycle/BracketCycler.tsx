...
function StaticVersion({
  ...
  return (
    ...
    >
      <span style={{ color: "var(--bc-text, var(--foreground, currentColor))" }}>
        {baseText}
      </span>
      <BracketSlot text={staticToken} animKey={0} scramble={false} />
    </span>
  );
}
...
function AnimatedVersion({
  ...
  const freezeMsRef     = useRef(15000);
  ...
  useEffect(() => {
    ...
    if (!isNaN(v)) freezeMsRef.current = v;
  }, []);
  ...
  const scramble = useCallback(
    async (target: string): Promise<void> => {
      ...
      for (let i = 0; i < SCRAMBLE_FRAMES; i++) {
        ...
        const text     = target.slice(0, revealed) + scrambleOf(len - revealed);

        setSlot((prev) => ({ text: text.slice(0, len), key: prev.key, scramble: true }));
        await sleep(frameMs);
      }
    },
    [sleep]
  ...
  const typewriter = useCallback(
    async (token: string): Promise<void> => {
      ...
      for (let i = 1; i <= token.length; i++) {
        ...
        if (frozenRef.current) { setTypePos(null); return; }
        setTypePos(i);
      }
      setTypePos(null);
    },
  ...
  const swapTo = useCallback(
    async (token: string, opts: { email?: boolean } = {}): Promise<void> => {
      if (frozenRef.current) return;
      ...
      if (opts.email) {
        ...
        setSlot((prev) => ({ text: "", key: prev.key + 1, scramble: false }));
        setSrText(token);
        await typewriter(token);
        if (frozenRef.current) return;
        // Commit the full string once typewriter finishes
        setSlot((prev) => ({ text: token, key: prev.key, scramble: false }));
      } else {
        setSlot((prev) => ({ text: token, key: prev.key + 1, scramble: false }));
        setSrText(token);
      }
    },
    [scramble, typewriter]
  ...
  const display =
    typePos !== null
      ? typeTargetRef.current.slice(0, typePos)
      : slot.text;
    ...
    >
      ...
      <span
        ...
        style={{
          ...
          fontFamily:  "var(--bc-font, ui-monospace, 'SF Mono', Consolas, monospace)",
          fontSize:    "0.9em",
